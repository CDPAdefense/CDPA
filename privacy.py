...
class Privacy:
    def __init__(self, config, dither_var):
        self.privacy_noise = config.privacy_noise

        if self.privacy_noise == 'laplace' or self.privacy_noise == 'jopeq_scalar':
            b_lap = 2 / config.epsilon
        ...
        if self.privacy_noise == 't' or self.privacy_noise == 'jopeq_vector':
            self.sigma_squared, self.nu = config.sigma_squared, config.nu
            cov_PPN = self.sigma_squared * np.eye(2)  # 't'
            self.noise = stats.multivariate_t(shape=cov_PPN, df=self.nu)
            if config.privacy_noise == 'jopeq_vector':  # subtract the cov_dither
