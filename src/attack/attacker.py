...
from networks.vgg_generator import VGGAutoEncoder
from networks.resnet_generator import ResNetAutoEncoder
...
class Attacker:
    ...
    def init_attacker_models(self, config):
        ...
        if config.postmodel == "VAE":
            # configs = [3, 8, 36, 3]
            configs =[2, 2, 3, 3, 3]
            bottleneck = True
            # self.generator = ResNetAutoEncoder(configs, bottleneck)
            self.generator = VGGAutoEncoder(configs)
        ...
        if config.postmodel == "VAE":
            ...
            # for k in torch.load("model_zoos/015.pth")["state_dict"].keys():
            #     print("_____________________________")
            #     print(k)
            state_dict = torch.load(config.joint_postmodel)
            new_state_dict = OrderedDict()
            for k, v in state_dict["state_dict"].items():
                # 检查每个键是否以 'module.' 为前缀。如果是，则去掉它。
                name = k[7:] if k.startswith('module.') else k
                new_state_dict[name] = v
            self.generator.load_state_dict(new_state_dict)
            # state_dict = torch.load("model_zoos/015.pth")["state_dict"]

            # load_dict("model_zoos/015.pth", self.generator)
        else:
            state_dict = torch.load(config.joint_postmodel)
    ...
    def grad_inv(self, grad, x, onehot, model, logger=None):
        ...
        for iters in range(self.T_max):
            def closure():
                ...
                pred = model(F.interpolate(dummy_data, scale_factor=self.sf, mode='nearest'))
                ...
                for gx, gy in zip(dummy_dy_dx, grad):
                    ...
                    # gx_for_lpips = gx.view(1, -1, gx.shape[0], gx.shape[1])
                    # gy_for_lpips = gy.view(1, -1, gy.shape[0], gy.shape[1])
                    # lpips_value = loss_fn_vgg.forward(gx_for_lpips, gy_for_lpips).item()
                    # grad_diff +=0.9* lpips_value
                # for gx, gy in zip(dummy_dy_dx, grad):
                #     grad_diff += (torch.tanh(gx) - torch.sign(gy) ** 2).sum()
    ...
    def joint_postprocess(self, x, y):
        with torch.no_grad():
            ...
            # refine1 = self.generator(self.resize(x))
        ...
        return refine1, 0.5*refine1+0.5*refine2
