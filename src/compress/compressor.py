...
import torch
import src.compress.integer_convert2 as ic

def inject_laplace_noise(tensor, epsilon):
    sensitivity = 2.0  # 敏感度为2，适用于张量中的每个元素

    # 计算隐私预算
    delta_epsilon = epsilon / sensitivity

    # 计算噪声的比例因子
    beta = sensitivity / delta_epsilon

    # 生成拉普拉斯噪声
    noise = torch.from_numpy(np.random.laplace(0, beta, size=tensor.size())).to(tensor.device)
...
class Topk:
    ...
    def compress(self, tensor, **kwargs):
        ...
        # # 使用DP
        # tensor = inject_laplace_noise(tensor, epsilon=10)
...
class BinaryConversion:
    ...
    def compress(self, tensor, **kwargs):
        ...
        Args,
            tensor (torch.tensor): the input tensor.
        """
        tensor_binary = ic.binary_convert(tensor,self.p )
        
        tensor_bi_converted = ic.binary_to_decimal2(tensor_binary)
        tensor_bi_converted = torch.tensor(tensor_bi_converted).cuda()
        tensor_bi_converted.requires_grad = True
        return tensor_bi_converted
