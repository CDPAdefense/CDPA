...
class Metrics:
    ...
    def evaluate(self, original_img, recon_img,y, logger):
        if self.half:
        ...
        for i in range(batch_size):
            ...
            im1_tensor = transforms.ToTensor()(Image.fromarray(im1)).unsqueeze(0).to(self.device)
            im2_tensor = transforms.ToTensor()(Image.fromarray(im2)).unsqueeze(0).to(self.device)
        
        # Compute LPIPS
            lpips_value = lpips_metric(im1_tensor, im2_tensor).item()
            ...
            logger.info("{:d} PSNR: {:.3f} SSIM: {:.3f} LPIPS {:.3f}".format(i, psnr_val, ssim_val,lpips_value))
