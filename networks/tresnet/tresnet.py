...
def conv2d_ABN(ni, nf, stride, activation="leaky_relu", kernel_size=3, activation_param=1e-2, groups=1):
    return nn.Sequential(
        ...
        nn.BatchNorm2d(nf),
        nn.ReLU(inplace=True) if activation == "relu" else nn.LeakyReLU(activation_param, inplace=True)
    )
...
class TResNet(Module):
    ...
    def __init__(self, layers, in_chans=3, num_classes=1000, width_factor=1.0,
        ...
        for m in self.modules():
            ...
            elif isinstance(m, nn.BatchNorm2d)  or isinstance(m,nn.BatchNorm2d) : # or isinstance(m, InPlaceABN) | isinstance(m,nn.LeakyReLU) or isinstance(m,nn.ReLU)
